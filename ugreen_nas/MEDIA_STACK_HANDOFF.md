...
| **Centralized EN/HI Audio & Subtitle Hardening** | Fixed generic Dual Audio custom format by strictly scoping to `Hindi Dual Audio` (`(?i)(?=.*\b(hindi)\b)(?=.*\b(dual.audio|dualaudio|dual)\b)`). Added `Foreign Dubs / Unwanted` custom format with -10,000 score penalty across all profiles to reject foreign dubs (Portuguese, Spanish, French, German, Russian). Configured Bazarr with English Language Profile, enabled zero-auth subtitle providers, linked Sonarr/Radarr SignalR feeds, and initiated automated upgrade of Friends S01-S03 to clean English Joy releases. | 2026-09-10 |
| **Plex Lean Maintenance & Event-Driven Scanning Mode** | Configured Plex into dormant lean mode tailored to Sonarr/Radarr pipeline. Disabled daily deep media analysis, periodic metadata refreshing, EPG guide syncing, and upgrade media analysis. Reduced maintenance window to 1 hour (13:00-14:00) dedicated exclusively to NVMe SQLite database optimization and rolling backups, keeping mechanical HDDs (`/volume1`) in 0 RPM deep hibernation until active streaming. | 2026-09-16 |
